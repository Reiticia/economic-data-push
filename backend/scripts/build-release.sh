...
文件:
  ${BIN_NAME}    后端可执行文件
  config.toml    服务配置 (端口、数据库、行情源、翻译)
  rules.toml     指标规则

直接运行:
  ./${BIN_NAME}
  默认读取当前目录的 config.toml 与 rules.toml，监听 0.0.0.0:8080，
  首次启动自动创建 data/market.db 并执行迁移。

环境变量:
  APP_CONFIG      指定配置文件路径 (默认 ./config.toml)
  RUST_LOG        日志级别，如 market_event_analyzer=info,tower_http=info
  OPENAI_API_KEY  事件名翻译 (translation.enabled = true 时必需)
  TE_API_KEY      仅 --backfill 历史补采需要

用 systemd 安装 (推荐，需要仓库的 deploy/deploy.sh):
  sudo ./deploy.sh deploy --binary /path/to/${BIN_NAME}
EOF
