...
密钥 (二选一，均可通过环境变量传入或写在 env 文件中):
  OPENAI_API_KEY  事件名翻译 (translation.enabled)
...
if [[ $# -gt 0 ]]; then
    case "$1" in
        deploy|install|upgrade|status|logs|uninstall|backfill) COMMAND="$1"; shift ;;
    esac
fi
...
cmd_deploy() {
    ...
    info "反向代理/防火墙示例: ufw allow ${PORT}/tcp  (生产建议前置 Nginx + TLS)"
}
...
cmd_status() {
    ...
    if command -v curl >/dev/null 2>&1; then
        ...
        [[ "${code}" == "200" ]] && log "健康检查 /health (port ${port}) -> 200" || warn "健康检查 /health (port ${port}) -> ${code:-无响应}"
    fi
    if [[ -f "${ENV_FILE}" ]]; then
        echo
